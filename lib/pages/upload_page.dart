...
class UploadPage extends StatelessWidget {
  ...
  Widget _buildMainContent(BuildContext context, UploadPageViewModel viewModel) {
    switch (viewModel.state) {
      ...
      case UploadPageState.filesSelected: return _buildFileManagementArea(context, viewModel);
      case UploadPageState.uploading:
      case UploadPageState.success:
      case UploadPageState.error: return _buildProgressArea(context, viewModel);
    }
  }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              const Text('選擇檔案以上傳', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
              const SizedBox(height: 8),
              const Text('支援格式: PDF, DOC, TXT, PNG, JPG', style: TextStyle(fontSize: 12, color: Colors.grey)),
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Text('已選檔案 (${viewModel.selectedFiles.length})', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
  ...
  Widget _buildProgressArea(BuildContext context, UploadPageViewModel viewModel) {
    ...
    switch (viewModel.state) {
      case UploadPageState.uploading:
        ...
        progressWidget = Column(children: [
      ...
      case UploadPageState.error:
        ...
        progressWidget = Text(viewModel.errorMessage ?? '發生未知錯誤', style: const TextStyle(color: Colors.red), textAlign: TextAlign.center);
        break;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(viewModel.statusMessage, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
            const SizedBox(height: 24),
            progressWidget,
          ],
        ),
  ...
  Widget _buildBottomButton(BuildContext context, UploadPageViewModel viewModel) {
    switch (viewModel.state) {
      ...
      case UploadPageState.uploading: return ElevatedButton(onPressed: null, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)),
        SizedBox(width: 16), Text('正在上傳...', style: TextStyle(fontSize: 16)),
      ]));
      case UploadPageState.error: return ElevatedButton.icon(onPressed: () => viewModel.reset(), icon: const Icon(Icons.refresh), label: const Text('重試', style: TextStyle(fontSize: 16)), style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), backgroundColor: Colors.orange));
      case UploadPageState.success: return ElevatedButton(onPressed: null, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), backgroundColor: Colors.green), child: const Text('上傳成功', style: TextStyle(fontSize: 16)));
    }
  }
